...
void printMatrix(T* matrix, int batch_size, int num_heads, int sequence_length, int embedding_dimension, int rowsToPrint, int colsToPrint) {
    ...
    for (int b = 0; b < batch_size; ++b) {
        for (int h = 0; h < num_heads; ++h) {
            for (int i = 0; i < rowsToPrint; i+=200) {
                for (int j = 0; j < colsToPrint; j+=200) {
...
int main() {
    ...
    const int embedding_dimension = 2048;

    const int block_size_columns =sequence_length;
    const int block_size_rows = embedding_dimension;
    ...
    dim3 block_dim(embedding_dimension, sequence_length);
    dim3 grid_dim((embedding_dimension + block_dim.x - 1)/block_dim.x, (sequence_length + block_dim.y - 1)/block_dim.y);
    ...
    float* output_matrix_host = new float[batch_size * num_heads * sequence_length * embedding_dimension * sizeof(float)];
    ...
    cudaEventRecord(stopP);  
    cudaEventSynchronize(stopP);

    float millisecondsP = 0;
    cudaEventElapsedTime(&millisecondsP, startP, stopP);  
    millisecondsP /= 1000;
